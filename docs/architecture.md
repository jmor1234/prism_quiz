...
- **State machine phases:** `intro` → `goals` → `loading_step` ↔ `answering` → `transition` → `loading_step` ↔ `answering` → `generating` → `result` (with `error` reachable from any async phase)
...
- **Synthetic step signal:** When user clicks "continue" at transition, a synthetic `[transition]` step is added to history so the agent knows the user opted in.
- **Double-click guard:** `isSubmitting` ref prevents duplicate step submissions on all async actions.
- **Persistence:** `lib/assessmentStorage.ts` with versioned schema (v2), graceful `QuotaExceededError` handling. Transition state persisted and restorable on refresh.
...
**Client storage:** variant-scoped localStorage keys (`prism-quiz:{variant}`). V1→V2 migration for root-cause. Assessment uses `prism-assessment` key with versioned schema (v2) storing name, steps, questionHistory (with transition entries), and result.
