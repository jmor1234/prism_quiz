...
- **Dynamic questions:** After Q1 (static health goal chips), every subsequent question + options are generated by the intake agent (Sonnet 4.6) based on accumulated context. Agent decides per-question whether multi-select or single-select (`multiSelect` boolean in schema).
...
- Model: Claude Sonnet 4.6 via `generateText` + `Output.object()` (structured output, modern AI SDK pattern)
- System prompt: role, context (cold traffic), 5 core areas to cover, optional depth guidance, voice (quiz not conversation)
- Knowledge: 3 files -- `knowledge.md` (bioenergetic framework for pattern recognition and option prediction), `intake_intelligence.md` (distilled symptom probing guide), `prism_process.md` (context for what information is worth gathering)
- Prompt caching: system message marked with `cacheControl: ephemeral` (5-min TTL). Calls 2-10 in a session hit the cache at ~99% hit rate.
- User message: formatted accumulated steps (or "Begin the intake" if empty)
- Output schema: `{ question, options[], freeTextPlaceholder, status, progressEstimate, multiSelect }`
...
└── api/
    ...
    │   ├── intake/
    │   │   ├── route.ts                Intake step generation (Sonnet 4.6, generateText + Output.object, cached)
