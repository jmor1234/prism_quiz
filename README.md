...
- **Config-driven leverage.** One `VariantConfig` per variant propagates to ~10 destinations (Zod schema, prompts, wizard state, UI dispatch, SEO, admin, PDF). Adding a variant is a config edit. The partner assessment (a structurally different 38-question B2B intake) shipped with a `getStorage(variant)` branch and a few small server modules; the rest is a single config file.
...
| **Partner assessment** | [`/quiz/prism-assessment`](https://prism-quiz.vercel.app/quiz/prism-assessment) (hidden) | B2B partner client bases | `bestlife-*` (same Redis, isolated keys) |
...
A 38-question deep health intake used by Prism's B2B partners. Hidden from the public `/quiz` listing; partners send their own audiences to it by direct URL. Reuses the entire engine with isolated storage (`bestlife-*` keys) and a dedicated admin at `/admin/best-life-harbor`, so partner submissions never mix with Prism's own funnel. No chat handoff in v1. **Total new engine code to ship this pillar: a `getStorage(variant)` branch in the existing route plus three small parallel server modules. The rest is a single config file.** That marginal cost is the payoff of the config-driven engine.

Originally built for a single partner under the slug `best-life-harbor` (and before that `best-life-care`). It was generalized once a second partner wanted it, so the public name carries no partner branding. Both old slugs still resolve — see the note on naming below.

Each partner gets a tagged link (`/quiz/prism-assessment?source=basedsupps`, or `utm_source` — both work), and the source is stored on the submission and shown in the admin. Onboarding a partner is handing them a URL; there's no code change and no allowlist per partner.

→ **See it live: [prism-quiz.vercel.app/quiz/prism-assessment](https://prism-quiz.vercel.app/quiz/prism-assessment)**
...
Two cross-cutting optionals any type can adopt: **`hideWhen`** (declarative skip-and-fill that cascades through the answer graph) and **`allowUnsure`** (third "Unsure" button on yes/no questions). The partner assessment (38 questions, complex multi-step skip cascades, conditional textareas) runs on the same engine the 11-question variants do, with no special-casing. The complexity rode entirely in config, which is the test the architecture had to pass.
...
| `bestlife-*` | prism-assessment | Same Redis instance, isolated key prefix |
| `assessment-*` | cold assessment | **Separate Redis DB** via `UPSTASH_ASSESSMENT_REDIS_REST_URL` |

Two different isolation patterns, deliberately. **The partner assessment** uses a key prefix on the same Redis instance because the operational cost of full separation wasn't worth it for what's essentially a partner-tenanted slice of the same product. **Assessment** gets its own Redis DB because reporting and audience analytics are entirely different from the standard funnel: different stakeholders, different metrics, different lifecycle. The pattern matches the *business* shape of each pillar, not a one-size-fits-all isolation rule.
...
### Why the partner assessment's internal names say `bestlife`

The pillar was renamed twice at the public layer (`best-life-care` → `best-life-harbor` → `prism-assessment`) as it generalized from one partner to any partner. The Redis key prefix (`bestlife-*`), the storage modules (`server/bestLife*.ts`), and the internal route paths (`/api/bestlife/*`, `/api/admin/best-life-harbor/*`) were deliberately **not** renamed: they hold live partner data, and renaming keys would mean a migration that buys nothing a user can see.

So a cold reader will find `lib/quiz/variants/prism-assessment.ts` writing to `bestlife-*` keys through routes named `best-life-harbor`. That divergence is intentional, not drift.

Old slugs stay resolvable through `SLUG_ALIASES` in [`lib/quiz/variants/index.ts`](./lib/quiz/variants/index.ts), because stored `record.variant` values are never rewritten — retries and result lookups need them to resolve. Resolution is a **single hop**, so every alias must point directly at the canonical slug, never at another alias. Anything that needs to ask "does this slug belong to the partner pillar?" must go through `isPrismAssessmentSlug()` rather than compare strings: it resolves aliases, and it is the one place that fact lives. A hardcoded slug list that falls out of date fails *silently*, routing partner submissions into the shared `quiz-*` keyspace where the standard admin filters them out.
...
app/                   Next.js App Router
  ...
  admin/               Password-gated dashboards (results, assessments, partner assessment, chats)
