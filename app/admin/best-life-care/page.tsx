...
function YesNoIndicator({
  ...
  const display = value === "unsure" ? "Unsure" : value ? "Yes" : "No";
  const color =
    value === "unsure"
  ...
  return (
    <div className="flex items-center justify-between py-1.5 border-b border-border/40 last:border-0">
      ...
      <span className={cn("text-[13px] font-semibold tracking-tight", color)}>
        {display}
      </span>
    </div>
  );
  ...
  switch (question.type) {
    ...
    case "yes_no": {
      if (question.conditionalFollowUp) {
        ...
        const a = compound?.answer;
        const indicatorValue: boolean | "unsure" =
          a === "unsure" ? "unsure" : a === true;
        const followUp = compound?.followUp ?? [];
        return (
          <div>
            <YesNoIndicator label={label} value={indicatorValue} />
            {a === true && followUp.length > 0 && (
      ...
      const v = value === "unsure" ? "unsure" : (value as boolean);
      return <YesNoIndicator label={label} value={v} />;
    }
    ...
    case "yes_no_with_text": {
      ...
      return (
        <div>
          <YesNoIndicator label={label} value={indicatorValue} />
          {showText && (
            <p className="text-sm text-foreground leading-relaxed whitespace-pre-wrap pl-2 pt-1.5 pb-1">
              {text}
            </p>
          )}
        </div>
