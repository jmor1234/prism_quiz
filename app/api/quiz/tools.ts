...
const searchTool = tool({
  ...
  execute: async ({ query, includeText, excludeText }) => {
    ...
    console.log(
      `[Search] "${query.slice(0, 80)}"${filterStr} → ${results.length} results · ${ms}ms · ~${tok} tok`
    );
    for (let i = 0; i < results.length; i++) {
      console.log(`  ${i + 1}. ${domain(results[i].url)} - ${results[i].title}`);
    }
...
const readTool = tool({
  ...
  execute: async ({ url, query }) => {
    ...
    console.log(
      `[Read]   ${domain(url)} → "${query.slice(0, 60)}" · ${chars} chars · ${ms}ms · ~${tok} tok`
    );
...
export const quizTools = { search: searchTool, read: readTool };
